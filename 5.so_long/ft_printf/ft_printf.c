...
/*   Updated: 2022/11/09 04:42:34 by rinacio          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_printf(const char *format, ...)
{
	...
	while (format[i])
	{
		...
		else if (ft_strchr2("0-.123456789#+ ", format[i + 1]))
		{
			flag = format[++i];
...
int	ft_parse(char c, va_list args, int n, char flag)
{
	...
	len = 0;
	if (c == 'i' || c == 'd')
		len += ft_putnbr2(va_arg(args, int), n, flag);
	else if (c == 'u')
		len += ft_putnbr2(va_arg(args, unsigned int), n, flag);
	else if (c == 'c')
		len += ft_putchar2(va_arg(args, int), n, flag);
	else if (c == 's')
		len += ft_putstr2(va_arg(args, char *), n, flag);
	else if (c == '%')
		len += write(1, "%", 1);
	else if (c == 'p')
